...
class CartPoleEnvEventsDebug(gym.Env):
	"""
	### Description
	...
	**Note:** While the ranges above denote the possible values for observation space of each element, it is not reflective of the allowed values of the state space in an unterminated episode. Particularly:
	...
	def step(self, action):
		...
		if not done:
			# reward = 1.0
			# reward = costheta
			reward = math.cos(float(abs(theta)) / float(abs(self.theta_threshold_radians)) / math.pi)
		...
		return event_tensor.numpy(), reward, done, {}
	...
	def tensorify_evflownet(self, events: list[Event]):
		...
		# spike_tensor = torch.zeros((4, self.screen_height, self.screen_width))
