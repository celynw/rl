...
def parse_args() -> argparse.Namespace:
	tasks = [name for name, obj in inspect.getmembers(rl.models) if inspect.isclass(obj) and name != "Base"]
	...
	group.add_argument("task", choices=tasks, metavar=f"TASK: {{{', '.join(tasks)}}}", help="Task to regress/estimate/solve")
	...
	Model = getattr(rl.models, args_[0].task)
